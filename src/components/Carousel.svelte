<script>
    ...
    let carousel;
		...
		onMount(() => {
		    logos.forEach(({ id, description }) => tippy(`.${id}`, makeTippyProps(description)));
		});
		...
		<div class="w-full lg:w-3/5 md:w-4/5">
				<Carousel perPage={{ 800: 6, 500: 3 }} easing="ease-in-out" autoplay=2000 dots={false} controls={false} bind:this={carousel}>
						{#each logos as logo (logo)}
								<div class="flex flex-col slide-content cursor-pointer {logo.id}" on:mouseenter={enter} on:mouseleave={leave}>
										<img class="h-12 object-scale-down m-3" src={logo.image} alt="{logo.name} logo" />
								</div>
						{/each}
				</Carousel>
		</div>
